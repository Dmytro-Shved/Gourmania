<!doctype html>
<html lang="en">
<head>
    <meta charset="UTF-8">
    <meta name="viewport"
          content="width=device-width, user-scalable=no, initial-scale=1.0, maximum-scale=1.0, minimum-scale=1.0">
    <meta http-equiv="X-UA-Compatible" content="ie=edge">
    <link rel="icon" type="image/x-icon" href="https://i.ibb.co/2jPbxvC/logo-round.png">
    <title>Gourmania | Recipes and more</title>

    {{-- Inknut Antiqua --}}
    <link rel="preconnect" href="https://fonts.googleapis.com">
    <link rel="preconnect" href="https://fonts.gstatic.com" crossorigin>
    <link href="https://fonts.googleapis.com/css2?family=Inknut+Antiqua:wght@300;400;500;600;700;800;900&family=Work+Sans:ital,wght@0,100..900;1,100..900&display=swap" rel="stylesheet">

    {{-- Inclusive Sans --}}
    <link rel="preconnect" href="https://fonts.googleapis.com">
    <link rel="preconnect" href="https://fonts.gstatic.com" crossorigin>
    <link href="https://fonts.googleapis.com/css2?family=Inclusive+Sans:ital@0;1&display=swap" rel="stylesheet">

    {{-- Inria Serif --}}
    <link rel="preconnect" href="https://fonts.googleapis.com">
    <link rel="preconnect" href="https://fonts.gstatic.com" crossorigin>
    <link href="https://fonts.googleapis.com/css2?family=Inria+Serif:ital,wght@0,300;0,400;0,700;1,300;1,400;1,700&display=swap" rel="stylesheet">

    @vite('resources/css/app.css')
</head>
<body>
<nav class="gourmania-bg text-white shadow-md py-3">
    <div class="container flex items-center">
        <!-- Logo -->
        <a href="/" class="flex items-center space-x-3">
            <img src="https://i.ibb.co/QXwnrsX/6041733416317207214.jpg" alt="Logo" class="w-79 h-79">
            <span class="text-3xl font-inknut">Gourmania |</span>
        </a>

        <!-- Links -->
        <ul class="flex">
            <li><a href="#" class="nav-link">Recipes</a></li>
            <li><a href="#" class="nav-link">Authors</a></li>
            <li><a href="#" class="nav-link">Basics</a></li>
            <li><a href="#" class="nav-link">News</a></li>
            <li><a href="#" class="nav-link">Cuisines</a></li>
        </ul>
    </div>
</nav>

<script src="../path/to/flowbite/dist/flowbite.min.js"></script>
</body>
</html>
