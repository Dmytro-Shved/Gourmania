...
class HomeService
{
    ...
    protected function getSections(): \Illuminate\Support\Collection
    {
        return cache()->remember('homepage-sections', now()->addDay(), function (){
    ...
    {
        return cache()->remember('statistics-section', now()->addDay(), function (){
            return [
    ...
    protected function getTopAuthors(int $limit = 12): Collection
    {
        return cache()->remember('top-authors', now()->addWeek(), function () use ($limit){
            return User::query()
                ->select([
                    'id',
                    'name',
